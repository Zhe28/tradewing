...
import type { dataList, record } from '../../env'
...
export const useAvaliableCustomerStore = defineStore('AvaliableCustomer', () => {
  ...
  function updateFilters(_dataList: keyof dataList) {
    ...
    data.value.forEach(function (item: dataList) {
      const record = { text: item[_dataList] as string, value: item[_dataList] as string }
      if (list.has(item[_dataList].toString())) return
      list.add(item[_dataList].toString())
